...
export const getUserHubPath = (request: string): string => getHubPath(`users/${FOR_USER || USER}/${request}`);
...
export const startServerForUser = (username: string): Promise<Response | null> => {
  return hubRequest('POST', `${USERS_PATH}/${username}/server`);
};
